import numpy as np
from qutip import destroy, coherent, wigner
...
# 0 态的 wigner 函数
vec = np.linspace(-5, 5, 202)  # 计算范围
W1 = wigner(psi=logic_0, xvec=vec, yvec=vec)

# 1 态的 wigner 函数
W2 = wigner(psi=logic_1, xvec=vec, yvec=vec)


# 绘制 wigner 函数图像
plt.figure(figsize=(6, 6))
plt.contourf(vec, vec, W1, 100, cmap=plt.get_cmap('RdBu'))

plt.figure(figsize=(6, 6))
plt.contourf(vec, vec, W2, 100, cmap=plt.get_cmap('RdBu'))
plt.show()
